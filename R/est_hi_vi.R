#' Function to estimate hybrid index using a variational algorithm for approximate posterior sampling
#'
...
#' @param p1 vector of allele frequencies for parental reference set 1 (one entry per locus).
...
#'}
est_hi_vi<-function(Gx=NULL,G0=NULL,G1=NULL,p0=NULL,p1=NULL,model="genotype",ploidy="diploid",pldat=NULL,
		 method="meanfield"){
	...
	if(model=="genotype" & ploidy=="diploid"){
		...
		fit<-rstan::vb(stanmodels$hi,data=dat,algorithm=method)
		...
		dat<-list(L=dim(Gx[[1]])[2],N=dim(Gx[[1]])[1],GL0=Gx[[1]],GL1=Gx[[2]],GL2=Gx[[3]]
			  ,P0=p0,P1=p1)
		fit<-rstan::vb(stanmodels$hi_gl,data=dat,algorithm=method)
		hi<-t(apply(rstan::extract(fit,"H")[[1]],2,quantile,probs=c(.5,.025,.05,.95,.975)))
	} else if(model=="ancestry" & ploidy=="diploid"){
		## diploid with known ancestry
		return(hiout)
		dat<-list(L=dim(Gx)[2],N=dim(Gx)[1],Z=Gx)
		fit<-rstan::vb(stanmodels$hi_z,data=dat,algorithm=method)
		hi<-t(apply(rstan::extract(fit,"H")[[1]],2,quantile,probs=c(.5,.025,.05,.95,.975)))
